import player, flask, json, os, downloader, test
...
def song_action():
    ...
    for x in group["songs"]:
        ...
        if x["name"]==song:
            song_index=i
            break
    print(action, group, song, song_index)
    if action=="minus":
...
@app.get("/songs")
def get_songs():
    return make_response([x[::-1].split(".", 1)[1][::-1] for x in os.listdir("songs") if "group_export" not in x])
...
def export_group():
    ...
    group=db.get("group_"+groupname)
    import test
    test.process_audio_batch(group["songs"], "group_export"+groupname+".mp3")
    return make_response(True)
